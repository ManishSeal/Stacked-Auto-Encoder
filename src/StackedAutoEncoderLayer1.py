...
def main():
    ...
    n_h = net_dims[1]
    n_fin = net_dims[2]learning_rate = 0.04
    num_iterations = 702
    activations = ["tanh","sigmoid"]
    ...
    costs, validation_costs, parameters, logs = two_layer_network(X=train_data,
                                                            Y=train_data,
    ...
    train_Pred = denoise(train_data, parameters, activations)
    test_Pred = denoise(test_data, parameters, activations)
    ...
    root = ""
    fig_name = "StackedLayer1figure_"+str(net_dims)+"_activations_"+str(activations)+\
    ...
    parameter_file = "StackedLayer1parameters_"+str(net_dims)+"_activations_"+\
    ...
    log_file = "StackedLayer1logs_"+str(net_dims)+"_activations_"+str(activations)+"_lr_"+str(learning_rate)+\
                        "_digits_"+str(len(digit_range))+\
                        "_ni_"+str(num_iterations)+\
                    "_train_size_"+str(train_data.shape[1])
